...
def add_donation_table():
    """Добавляем таблицу для пожертвований"""
    from app import create_app
    from app import db
    ...
    with app.app_context():
        ...
        columns = [col["name"] for col in inspector.get_columns("donations")]
        print(f"Столбцы в таблице 'donations': {columns}")


# Добавьте вызов этой функции в конце файла или запустите отдельно
